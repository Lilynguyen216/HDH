...
/* 
Initialize an array frames to store the pages in memory
Initialize an array reference_bits to store the reference bits
Set all reference_bits to -1
Set pointer to 0

Function ClockAlgorithm(page):
    While true:
        If reference_bits[pointer] == 0:
            Replace frames[pointer] with page
            Set reference_bits[pointer] to 1
            Move pointer to the next position (pointer = (pointer + 1) % size of frames)
            Break
        Else:
            Set reference_bits[pointer] to 0
            Move pointer to the next position (pointer = (pointer + 1) % size of frames)
/////////////////////////////////////
while (true) {
            ...
            if (checkEmptySecondChance === false) {
                if (this.secondChance[this.pointer] === 0) {
                    Replace frames[this.pointer] with page
                    return this.pointer;
                }
...
const TXT_ALWAYS_CLOCK = "WHILE true";
const TXT_ALWAYS_CHECK_EMPTY = "    let checkEmptySecondChance = this.secondChance.includes(-1);";
...
class scene2 extends Phaser.Scene {
    ...
    create() {
        ...
        const createButton = (x, y, linkTxt) => {
            let btn = this.add.image(x, y, linkTxt).setInteractive({ useHandCursor: true }).setDisplaySize(btn_WIDTH * 1.3, btn_WIDTH * 0.8);
            ...
            shape.fillRoundedRect(
                ...
                btn_WIDTH * 1.3,
                btn_WIDTH * 0.8,
                8 // Bán kính bo tròn, bạn có thể điều chỉnh giá trị này
            );
